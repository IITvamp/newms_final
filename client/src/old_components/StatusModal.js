...
const StatusModal = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // todo
    if (status.onEdit) {
      dispatch(updatePost({ title, content, images,url, auth, status }));
    } else {
      dispatch(createPost({ title, content, images,url, auth, socket }));
    }
    setTitle("");
    setContent("");
    setURL("");
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="status_body">
          ...
          <textarea
            ...
            value={url}
            name="url"
            placeholder={`${auth.user.username}, What's the url`}
            style={{
              filter: theme ? "invert(1)" : "invert(0)",
              color: theme ? "white" : "#111",
              background: theme ? "rgb(0,0,0,0.3)" : "",
